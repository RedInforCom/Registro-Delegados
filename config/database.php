...
// Cargar clave de registro desde archivo externo (si existe) para poder cambiarla sin editar este archivo
$reg_file = __DIR__ . '/registration_key.php';
if (file_exists($reg_file)) {
    // registration_key.php debe devolver la cadena con la clave
    $REGISTRATION_KEY = include $reg_file;
    if (!is_string($REGISTRATION_KEY) || $REGISTRATION_KEY === '') {
        // valor por defecto si el archivo está vacío o no devuelve string
        $REGISTRATION_KEY = '#Psico.#Peru26';
    }
} else {
    // valor por defecto (anteriomente hardcodeado)
    $REGISTRATION_KEY = '#Psico.#Peru26';
}
define('REGISTRATION_KEY', $REGISTRATION_KEY);
